    ...
    async function fetchData() {
      try {
        ...
        if (
          ...
          setErrorMessage(
            `Dashboard API取得失敁E summary=${summaryRes.status}, posts=${postsRes.status}, attention=${attentionRes.status}, trend=${roiTrendRes.status}`
          );
          setSummary(null);
          setPosts([]);
        ...
        setErrorMessage(
          "Dashboard API取得に失敗しました。backend起動�EJWT・CORSを確認してください、E
        );
      } finally {
        setLoading(false);
      }
  ...
  const summaryCards = [
    {
      ...
      sub: `目樁E${targetRoiPoints.toLocaleString()}P`,
    },
    {
      title: "財務インパクチE,
      ...
      <main style={styles.page}>
        <section style={styles.container}>
          ...
          >
            <div>
              ...
              <p style={styles.description}>
                現場の挑戦行動を、AI補完�E上司評価・ROI換算で経営判断へ接続します、E
              </p>
            </div>
          ...
          <div style={styles.twoColumn}>
            <Panel title="KGI進捁E tag="KGI">
              <div style={styles.kgiTop}>
                ...
                <div style={styles.badge}>
                  目樁E{formatMoney(summary?.target_value ?? 600000000)}
                </div>
              </div>
            ...
            <Panel title="価値変換ロジチE��" tag="LOGIC">
              <FlowItem
                step="01"
                title="現場の行動"
                text="挑戦・改喁E�E共有を短斁E��入劁E
              />
              <FlowItem
                step="02"
                title="AI補完＋上司確誁E
                text="AIが意味づけし、人が妥当性を確誁E
              />
              <FlowItem
                step="03"
                title="経営判断へ接綁E
                ...
                filteredPosts.map((post) => (
                  <div key={post.id} style={styles.aiInsightCard}>
                    ...
                    <div style={styles.detailBlock}>
                      <span style={styles.detailLabel}>社員入劁E/span>
                      <p style={styles.aiBehavior}>{post.behavior}</p>
                    </div>

                    <div style={styles.detailGrid}>
                      <div style={styles.aiCommentBox}>
                        <span style={styles.aiLabel}>AI刁E��</span>
                        <p>{post.ai_comment || "AIコメント未生�E"}</p>
                      </div>

                      <div style={styles.managerCommentBox}>
                        <span style={styles.managerLabel}>上司コメンチE/span>
                        <p>{post.manager_comment || "上司コメント未入劁E}</p>
                      </div>
                    </div>
                  </div>
                ))
          ...
          <div style={styles.twoColumn}>
            <Panel title="部門間評価乖離" tag="BIAS ALERT">
              <div style={styles.biasAlertGrid}>
                {departmentBiasAlerts.length === 0 ? (
                  <div style={styles.emptyBox}>評価乖離アラート�Eありません、E/div>
                ) : (
            ...
            <Panel title="ROIトレンチE tag="TREND">
              <div style={styles.chartBox}>
                {roiTrendData.length === 0 ? (
                  <div style={styles.emptyBox}>
                    ROIトレンドデータがまだありません、E
                  </div>
                ) : (
...
function formatMoney(value: number) {
  if (value >= 100000000) {
    return `${(value / 100000000).toFixed(2)}儁E�E`;
  }
  return `${Math.round(value / 10000).toLocaleString()}丁E�E`;
}

function statusLabel(status: string) {
  if (status === "approved") return "承認済み";
  if (status === "rejected") return "差戻ぁE;
  return "未承誁E;
}
