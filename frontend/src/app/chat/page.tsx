...
export default function ChatPage() {
  ...
  const [messages, setMessages] = useState<ChatDisplay[]>([
    {
      type: 'ai',
      content: 'こんにちは！私は経験豊富なファイナンシャルプランナーAIです。資産管理、人生設計、住宅購入、教育費、老後資金など、お金に関する質問にお答えします。お気軽に何でもご相談ください！',
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/chat/free-chat', {
        method: 'POST',
        headers: {
      ...
      <main className="flex-1 flex flex-col max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
          <section className="flex h-full p-6">
            <div className="flex-1 flex flex-col mr-4">
              ...
              {/* 推奨サジェストメッセージ */}
              <div className="mb-4 bg-blue-50 border-l-4 border-blue-400 text-blue-800 p-4 rounded shadow">
                <p className="text-sm mb-2 font-bold">
                  💡 自由に質問してください！AIが資産管理や人生設計のアドバイスをします
                </p>
                <p className="text-sm">
                  例：
                  <strong>「住宅購入のベストタイミングは？」</strong>
                  <strong>「老後資金はいくら必要？」</strong>
                  <strong>「教育費の準備方法は？」</strong>
                  <strong>「資産運用を始めたい」</strong>
                </p>
              </div>
