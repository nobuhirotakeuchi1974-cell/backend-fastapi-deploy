...
interface Career {
  ...
  total_annual_income?: number;  // その年の総年収
}
...
export default function CareerPage() {
  ...
  useEffect(() => {
    ...
    fetchIncomes();
    fetchUserAge();
    fetchBaseSettings();  // ベース設定をサーバーから取得
    ...
    try {
      ...
      if (response.ok) {
        ...
        setBaseSettings(prev => ({
          ...prev,
          current_income: data.current_income?.toString() || '5000000',
          base_increase_rate: data.base_salary_increase_rate?.toString() || '2',
          retirement_age: data.retirement_age?.toString() || '65',
          current_age: data.current_age?.toString() || prev.current_age
        }));
      }
    } catch (error) {
      console.error('ベース設定の取得に失敗しました:', error);
    }
  };
  ...
  const saveBaseSettings = async () => {
    try {
      ...
      await fetch('http://localhost:8000/api/auth/settings', {
        ...
        body: JSON.stringify({
          retirement_age: parseInt(baseSettings.retirement_age) || 65,
          base_salary_increase_rate: parseInt(baseSettings.base_increase_rate) || 2,
          current_income: parseInt(baseSettings.current_income) || 5000000,
          current_age: parseInt(baseSettings.current_age) || null
        })
      });
      console.log('ベース設定を保存しました');
    } catch (error) {
      console.error('ベース設定の保存に失敗:', error);
    }
  };
  ...
  const calculateIncomeProjection = (sideJobIncome: number) => {
    ...
    // 副業イベントを抽出
    const sideJobEvents = careers.filter(c => 
      c.career_type === '副業開始' || 
      c.career_type === '副業' ||
      c.career_type.includes('副業')
    );
    ...
    for (let year = currentYear; year <= retirementYear; year++) {
      ...
      if (year === currentYear) {
        ...
        // 副業イベントは年収には反映せず、副業収入のみに反映する
        const isSideJobEvent = event.career_type === '副業開始' || 
                              event.career_type === '副業' ||
                              event.career_type.includes('副業');
      ...
      // その年に開始している副業収入を計算
      let yearSideJobIncome = 0;
      sideJobEvents.forEach(job => {
        if (job.event_year && job.event_year <= year && job.expected_income) {
          yearSideJobIncome += job.expected_income;
        }
      });
      
      const totalIncome = yearIncome + yearSideJobIncome;
      ...
      projection.push({
        ...
        income: yearIncome,
        sideJobIncome: yearSideJobIncome,
  ...
  useEffect(() => {
    const saveProjection = async () => {
      ...
      try {
        const token = localStorage.getItem('access_token');
        await fetch('http://localhost:8000/api/career/update-projection', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
          },
          body: JSON.stringify({ projections: projection })
        });
        console.log('年収推移データを保存しました');
      } catch (error) {
        console.error('年収推移データの保存に失敗:', error);
      }
    };
    
    saveProjection();
  }, [JSON.stringify(projection)]);  // projectionが変わったら保存
          ...
          <div className="grid grid-cols-4 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  setBaseSettings({...baseSettings, base_increase_rate: e.target.value});
                }}
                onBlur={saveBaseSettings}
                className="w-full px-3 py-2 border border-gray-300 rounded"
                placeholder="例: 2"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => {
                  setBaseSettings({...baseSettings, retirement_age: e.target.value});
                }}
                onBlur={saveBaseSettings}
                className="w-full px-3 py-2 border border-gray-300 rounded"
                placeholder="例: 65"
